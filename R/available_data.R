...
ui <- fluidPage(
  mainPanel(
    ...
    absolutePanel(top = 60, left = 20,
                  #checkboxInput("markers", "SDG", FALSE),
                  checkboxInput("heat", "Refugees", FALSE)
    )
  ))
...
server <- function(input, output, session) {
  ...
  output$mymap <- renderLeaflet({
    leaflet(data) %>%
      setView(lng = 25, lat = 26, zoom = 3)  %>% #setting the view over ~ center of North America
      addTiles() %>%
